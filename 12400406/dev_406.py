...
def _load_ground_truth() -> pd.DataFrame:
    ground_truth_events = pd.read_pickle(DATA_DIR / "blinker_results.pkl")["frames"][
        "blinkFits"
    ]
    ...
    return ground_truth_events.sort_values(by="start_blink").reset_index(drop=True)
...
def _load_detections() -> pd.DataFrame:
    ...
    return detection.sort_values(by="start_blink").reset_index(drop=True)
...
def main() -> None:
    ...
    raw = mne.io.read_raw_fif(DATA_DIR / "12400406.fif", preload=True)
    # raw.crop(0, 12.0)
