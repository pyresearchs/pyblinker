...
Run
---
Run this file directly (``python step1bi_tutorial_validate_get_blink_positions.py``)
from the repository root. If everything matches, you’ll see
``✅ Validation PASSED``. Otherwise the script raises an ``AssertionError``
and prints diffs.
"""
...
def _import_dependencies() -> tuple[Path, object, object]:
...
REPO_ROOT, get_blink_position, load_matlab_data = _import_dependencies()
