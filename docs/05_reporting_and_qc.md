...
* **Feature/Change**: The EEG epoch landmark tutorial now renders EEG traces as low-opacity lines with scatter points, includes a horizontal zero-reference line, and pads each blink window to include the last available EEG landmark before plotting. Landmarks still come from exploded `epochs.metadata` fields like `start__left_base__eeg` and `x_intersect__eeg`.
