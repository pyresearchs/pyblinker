...
* **Feature/Change**: Added a dedicated plotting helper that renders one plot per blink in an HTML report, using EEG signals plus optional EAR overlays and the refined landmark points (e.g., `start__left_base__eeg`, `x_intersect__eeg`) sourced from exploded `epochs.metadata`.
