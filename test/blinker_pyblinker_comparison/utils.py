...
def load_matlab_blink_positions(path: str | Path) -> np.ndarray:
    ...
    if "signalData" not in blinks.dtype.names:
        raise KeyError(
            f"'signalData' not found in blinks structure. Fields: {blinks.dtype.names}"
        )
    ...
    if not hasattr(first_signal, "dtype") or first_signal.dtype.names is None:
        raise ValueError(
            f"Could not extract a valid struct from signalData. Type: {type(first_signal)}"
        )

    if "blinkPositions" not in first_signal.dtype.names:
        raise KeyError(
            f"'blinkPositions' not found in signalData. Fields: {first_signal.dtype.names}"
        )
    ...
    if arr.shape[0] != 2:
        if arr.ndim == 2 and arr.shape[1] == 2:
            ...
            raise ValueError(
                f"Unexpected blinkPositions shape {arr.shape}; expected (2, N)"
            )
