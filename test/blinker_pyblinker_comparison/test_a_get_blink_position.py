...
class TestCompareGetBlinkPosition(unittest.TestCase):
    def test_compare_blink_positions_with_matlab(self):
        ...
        self.assertTrue(
            mat_expected.exists(), f"Missing MATLAB expected file: {mat_expected}"
        )
        ...
        data = raw.get_data()
        self.assertEqual(
            data.shape[0], 1, f"Expected single channel, got shape {data.shape}"
        )
        ...
        self.assertEqual(
            len(df_py),
            len(df_mat),
            f"Different number of detected blinks: py={len(df_py)} mat={len(df_mat)}",
        )
