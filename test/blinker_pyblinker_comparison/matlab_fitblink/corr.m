...
%         'Weights' An N-by-1 vector of nonnegative numbers to be used as
%                   weights. Default is to weight all observations equally.
%
...
%   small sample sizes), or large-sample approximations. When there are
%   weights, p-values are not computed (set to NaN).
%
...
% Only x given, compute pairwise correlations
if (nargin < 2) || ischar(varargin{1})
    corrXX = true;
    y = x;
    p2 = p1;

% Both x and y given, compute the pairwise cross correlations
else
    y = varargin{1};
...
pnames = {'type'  'rows' 'tail' 'weights'};
dflts  = {'p'     'a'    'both' []};
[type,rows,tail,wts] = internal.stats.parseArgs(pnames,dflts,varargin{:});

% Validate the weights parameter
if ~isempty(wts)
    validateattributes(wts,{'double' 'single'}, {'size',[n 1],'nonnegative'},'corr','Weights');
end
...
switch rows
case 'a' % 'all'
    % Missing values are left in, so all cols have the same length, but
...
case 'p' % 'pairwise'
    ...
    % different length, we'll have to see. However, any NaN weights can be
    % removed up front.
    if ~isempty(wts)
        ok = (wts>0);
        n = sum(ok);
...
case 'k' % Kendall rank correlation
    if isempty(wts) && isa(x,"double") && isa(y,"double")
        corrFun = @internal.stats.corrKendallFast;
    else
...
if corrXX
    if nargout<2
        coef = corrFun(rows,tail,wts,x);
    else
        [coef,pval] = corrFun(rows,tail,wts,x);
        pval = (pval + pval')/2;
    end
    ...
    if nargout<2
        coef = corrFun(rows,tail,wts,x,y);
    else
        [coef,pval] = corrFun(rows,tail,wts,x,y);
    end
end
