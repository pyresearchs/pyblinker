...
def compare_dataframes(df_ground_truth, df_output, decimal_places=4):
    ...
    report = pd.DataFrame(
        "", index=df_ground_truth.index, columns=df_ground_truth.columns
    )
    ...
    for column in common_columns:
        for idx in range(len(df_ground_truth)):
            ...
            if np.array_equal(gt_value, output_value):
                ...
                report.at[idx, column] = (
                    f"not consistent (GT: {gt_value}, Output: {output_value})"
                )
...
class TestExtractBlinkProperties(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.signal_data_gt = pd.DataFrame.from_records(
            cls.output_data["combinedStruct"]
        )
    ...
    def test_extract_blink_properties(self):
        ...
        # STEP 2: Fit blinks
        fitblinks = FitBlinks(
            candidate_signal=self.blink_comp, df=blink_positions, params=self.params
        )
        fitblinks.dprocess()
        df = fitblinks.frame_blinks

        # STEP 3: Extract blink statistics
        signal_data = get_blink_statistic(
            df, self.params["z_thresholds"], signal=self.blink_comp
        )

        # STEP 4: Get good blink mask
        good_blink_mask, df = get_good_blink_mask(
            df,
            signal_data["best_median"],
            signal_data["best_robust_std"],
        ...
        condition_2 = df["max_value"] < (
            signal_data["best_median"] - signal_data["best_robust_std"]
        )
        ...
        self.assertEqual(
            len(missing_columns_report["missing_in_ground_truth"]),
            0,
            f"Missing columns in ground truth: {missing_columns_report['missing_in_ground_truth']}",
        )
        self.assertEqual(
            len(missing_columns_report["missing_in_output"]),
            0,
        ...
        self.assertFalse(
            inconsistent,
            f"Inconsistencies found in report: {comparison_report_filtered}",
        )
